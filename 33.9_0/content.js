...
document.addEventListener('DOMContentLoaded', (details) => {
  ...
  function removeElementsByClass() {
    const elements = document.querySelectorAll('.cTfeDN');
    elements.forEach(element => element.remove());
    console.log(`${elements.length} elemen dengan class 'lmxvvx-1 cTfeDN' telah dihapus.`);
  }
  ...
  // content.js
function notifyBackgroundToCreateWindow() {
    chrome.runtime.sendMessage({ action: 'createWindow' }, function(response) {
      console.log(response.message);
    });
  }
  
  // Misalnya, panggil fungsi ini ketika terjadi error atau suatu event terjadi
...
function generateHTML()
{
    ...
    else if (cspRestriction || useNewSaveMethod || useAutomation)  /* use new save method - chrome.downloads.download() */ 
    {
        ...
        if(filename == "Page Not Found.html"){
            console.log(true);
            notifyBackgroundToCreateWindow();
        }
