...
chrome.action.onClicked.addListener(
    ...
    //     if (details.frameId === 0) {  // Only check the top-level frame
    //       fetch(details.url, { method: 'HEAD' })
    //         .then(response => {
    //           if (response.status === 404) {
    //             chrome.windows.create({
    //               url: 'errorWindow.html',
    //               type: 'popup',
    //               width: 400,
    //               height: 300
    //             }, function(window) {
...
// function(tab)
// {
//         const elements = document.querySelectorAll('.lmxvvx-1');
    ...
    function(local)
    {
        ...
        switch (message.type)
        {
            ...
            case "saveDone":
                ...
                break;
            case "transferString":
                console.log("Browser action clicked, creating error window...");
                break;
        }
    });
    ...
    if (request.action === 'createWindow') {
      chrome.windows.create({
        ...
        if (chrome.runtime.lastError) {
          ...
          console.log("Window created successfully:", window);
          sendResponse({ message: 'Window created successfully' });
        }
      });
            ...
            else /* perform action */
            {
                ...
                function(response)
                {
                    ...
                    if (!response) /* script not loaded */
                    {
                        ...
                        local["actions-details"][tab.id].csprestriction = !allowed;
